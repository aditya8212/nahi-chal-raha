...
if(gameState === "play"){
	...
	for (var i = 0; i < laserarr.length; i=i+1) {
...
function collision() {
	if(laser.body.position.y-john.y < laser.height/2+john.height/2 &&
	  john.y-laser.body.position.y < laser.height/2+john.height/2  &&
	  laser.body.position.x-john.x < laser.width/2+john.width/2 &&
	   john.x-laser.body.position.x < laser.width/2+john.width/2){
		  return true;
	}
	else{
		return false;
